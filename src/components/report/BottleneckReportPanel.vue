...
function sanitizeReportHtml(html?: string | null): string {
  ...
  const isMarkdown = !html.trim().startsWith('<');
  const htmlContent = isMarkdown ? (marked(html) as string) : html;
  ...
  <section class="bnc-report">
    ...
    <template v-else>
      <template v-if="report.reportV1">
        ...
        <ReportV1BottleneckReportView ref="reportViewRef" :report="report.reportV1" :case-id="report.caseId" />
      </template>
