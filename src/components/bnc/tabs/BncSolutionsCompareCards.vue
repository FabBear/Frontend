...
function lotBadgeLabel(group: BncActionSpecGroup): string {
  ...
  if (n === 0 && isHotAction(group.action)) return 'SuperHotLot 적용';
  if (n === 0) return '우선순위 조정 적용';
...
function lotBadgeDesc(group: BncActionSpecGroup): string {
  const n = group.lots.length;
  if (n === 0) return '전략은 제공되었지만 개별 lot 상세는 API 응답에 포함되지 않았습니다.';
...
function intervalText(plan: BncActionPlan): string {
  if (plan.actionSpec?.intervalText) return plan.actionSpec.intervalText;
  if (plan.actionSpec?.intervalPct !== undefined) return `+${plan.actionSpec.intervalPct.toFixed(1)}%`;
  if (plan.actionSpec?.intervalDeltaMin !== undefined) return `+${plan.actionSpec.intervalDeltaMin.toFixed(1)}분`;
  return '조정값 없음';
}

function intervalDesc(plan: BncActionPlan): string {
  if (plan.actionSpec?.intervalPct !== undefined || plan.actionSpec?.intervalDeltaMin !== undefined) {
    return `신규 lot 투입 간격을 ${intervalText(plan)} 조정해 혼잡을 완화합니다.`;
  }
  return plan.summary || 'Lot 투입 간격 조정값이 제공되지 않았습니다.';
}
  ...
  <section class="bnc-solutions__compare">
    ...
    <div class="bnc-solutions__cards">
      ...
      >
        ...
        <div class="bnc-solutions__op-section">
          <span class="bnc-solutions__op-label">Lot 투입 간격 (Release Interval)</span>
          <span class="bnc-solutions__interval-val">{{ intervalText(plan) }}</span>
          <p class="bnc-solutions__op-desc">{{ intervalDesc(plan) }}</p>
        </div>
        ...
        <div class="bnc-solutions__lot-section">
          ...
          <template v-else-if="plan.actionSpec?.lotGroups?.length">
            ...
            <div v-if="totalLots(plan) > 0" class="bnc-solutions__lot-list">
